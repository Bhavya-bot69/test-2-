...
function TeamsTab({ teams = [], venues = [], onTeamsChange = () => {} }) {
  ...
  const handleExportPDF = () => {
    ...
    teams.forEach((team, index) => {
      ...
      yPosition += 6;
      doc.text(`Venue: ${venues.find((v) => v.id === team.venueId)?.name || "N/A"}`, 20, yPosition);
      yPosition += 6;
  ...
  const handleExportExcel = () => {
    ...
    const headers = ["Team Name", "Project Title", "Leader Name", "Leader Email", "Venue", "Category"];
    const csvRows = [
      headers.join(','),
      ...teams.map(team => [
        ...
        escapeCsvCell(venues.find(v => v.id === team.venueId)?.name || 'N/A'),
  ...
  const handleFileImport = (event) => {
      ...
      reader.onload = (e) => {
          ...
          try {
              ...
              const newTeams = json.map(row => {
                  const venue = venues.find(v => v.name.toLowerCase() === (row.Venue || '').toLowerCase());
                  
                  if (!row['Team Name'] || !row['Leader Name'] || !row['Leader Email']) {
                      console.warn("Skipping a row due to missing required fields:", row);
                      return null;
                  }
              ...
              if (newTeams.length > 0) {
                ...
                onTeamsChange(updatedTeams);
                console.log(`${newTeams.length} teams imported successfully.`);
              }
          } catch (error) {
              console.error("Error parsing Excel file:", error);
              alert("There was an error processing the Excel file. Please ensure it is correctly formatted.");
          }
      };
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ background: "linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)" }}>
              ...
              <TableCell sx={{ fontWeight: 700, color: "#1e293b", fontSize: "0.95rem" }}>Venue</TableCell>
            ...
            {teams.length === 0 ? (
              ...
              teams.map((team) => (
                <TableRow key={team.id} sx={{ "&:hover": { backgroundColor: "#f8fafc" } }}>
                  ...
                  <TableCell sx={{ color: "#334155" }}>{getVenueName(team.venueId)}</TableCell>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <FormControl fullWidth margin="normal">
            ...
            <Select value={currentTeam.venueId} onChange={(e) => setCurrentTeam({ ...currentTeam, venueId: e.target.value })} label="Venue">
